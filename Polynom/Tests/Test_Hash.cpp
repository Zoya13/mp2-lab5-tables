...
TEST(Hash, can_create_a_table)
{
	ASSERT_NO_THROW(Hash A);
}

TEST(Hash, can_insert_a_row)
{
	Hash H;
	Polynom A;
	row r;
...
TEST(Hash, can_not_find_a_missing_polynom)
{
	struct m* pol;
	pol = (struct m*)malloc(5 * (sizeof(int) + sizeof(double)));
	for (int i = 0; i < 3; i++) {
...
TEST(Hash, can_delete_a_polynom)
{
	struct m* pol;
	pol = (struct m*)malloc(5 * (sizeof(int) + sizeof(double)));
	for (int i = 0; i < 3; i++) {
