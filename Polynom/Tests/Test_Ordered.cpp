...
TEST(Ordered, can_create_a_table)
{
	ASSERT_NO_THROW(Ordered A);
}

TEST(Ordered, can_insert_a_row)
{
	Ordered H;
	Polynom A;
	row r;
...
TEST(Ordered, can_find_a_polynom)
{
	struct m* pol;
...
TEST(Ordered, can_not_find_a_missing_polynom)
{
	struct m* pol;
	pol = (struct m*)malloc(5 * (sizeof(int) + sizeof(double)));
	for (int i = 0; i < 3; i++) {
...
TEST(Ordered, can_delete_a_polynom)
{
	struct m* pol;
	pol = (struct m*)malloc(5 * (sizeof(int) + sizeof(double)));
	for (int i = 0; i < 3; i++) {
